...
export const { baseTaskFetchStart, baseTaskFetchFailure, baseTasksFetchSuccess,baseTaskFetchSuccess } = baseTaskSlice.actions;

export const getFetchBaseTasks = () => async (dispatch:any) => {
  dispatch(baseTaskFetchStart)
  fetchApi.GetFetch<BaseTask>('http://localhost:8008/logtool/base_tasks')
    .then((res) => {
...
export const getFetchBaseTask = (id:number) => async (dispatch:any) => {
  dispatch(baseTaskFetchStart)
  fetchApi.GetFetch(`http://localhost:8008/logtool/base_tasks?params=${id}`)
    .then((res) => {
...
export const createFetchBaseTask = (data:AddBaseTask) => async (dispatch:any) => {
  dispatch(baseTaskFetchStart)
  fetchApi.PostFetch<AddBaseTask>('http://localhost:8008/logtool/base_tasks',data)
    .then((res) => {
...
export const updateFetchBaseTask = (data:AddBaseTask) => async (dispatch:any) => {
  dispatch(baseTaskFetchStart)
  fetchApi.PutFetch<AddBaseTask>('http://localhost:8008/logtool/base_tasks',data)
    .then((res) => {
      dispatch(getFetchBaseTasks())
    }).catch((err) => {
      dispatch(baseTaskFetchFailure(err))
...
export const deleteFetchBaseTask = (id:number) => async (dispatch:any) => {
  dispatch(baseTaskFetchStart)
  fetchApi.DeleteFetch(`http://localhost:8008/logtool/base_tasks?params=${id}`)
    .then((res) => {
      dispatch(getFetchBaseTasks())
    }).catch((err) => {
      dispatch(baseTaskFetchFailure(err))
