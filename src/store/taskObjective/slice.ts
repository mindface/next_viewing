...
export const { taskObjectiveFetchStart, taskObjectiveFetchFailure, taskObjectiveFetchSuccess } =
  taskObjectiveSlice.actions

export const getFetchTaskObjective = () => async (dispatch: any) => {
  dispatch(taskObjectiveFetchStart)
  fetchApi
    .GetFetch<TaskObjective>('http://localhost:8008/logtool/task_objective')
...
export const createFetchObjective = (data: AddTaskObjective) => async (dispatch: any) => {
  dispatch(taskObjectiveFetchStart)
  fetchApi
    .PostFetch<AddTaskObjective>('http://localhost:8008/logtool/task_objective', data)
...
export const updateFetchObjective = (data: AddTaskObjective) => async (dispatch: any) => {
  dispatch(taskObjectiveFetchStart)
  fetchApi
    .PutFetch<AddTaskObjective>('http://localhost:8008/logtool/task_objective', data)
    .then((res: AddTaskObjective) => {
      console.log(res)
      dispatch(getFetchTaskObjective())
    })
...
export const deleteFetchObjective = (id: number) => async (dispatch: any) => {
  dispatch(taskObjectiveFetchStart)
  fetchApi
    .DeleteFetch(`http://localhost:8008/logtool/task_objective?params=${id}`)
    .then((res: AddTaskObjective) => {
      dispatch(getFetchTaskObjective())
    })
    .catch((err) => {
