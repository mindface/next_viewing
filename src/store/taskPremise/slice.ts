...
export const {
  taskPremiseFetchStart,
  taskPremiseFetchFailure,
  taskPremiseFetchSuccess,
  taskPremiseDiscFetchSuccess,
} = taskPremiseSlice.actions

export const getFetchTaskPremise = () => async (dispatch: any) => {
  dispatch(taskPremiseFetchStart)
  fetchApi
    .GetFetch<TaskPremise>('http://localhost:8008/logtool/task_premise/')
    .then((res) => {
...
export const getFetchTaskPremiseDisc = (id: number) => (dispatch: Dispatch<any>) => {
  ...
  return new Promise((resolve, rejects) => {
    fetchApi
      .GetFetch<TaskPremise>(`http://localhost:8008/logtool/task_premise_disc?params=${id}`)
      .then((res) => {
...
export const createFetchTaskPremise = (data: AddTaskPremise) => async (dispatch: any) => {
  dispatch(taskPremiseFetchStart)
  fetchApi
    .PostFetch<AddTaskPremise>('http://localhost:8008/logtool/task_premise/', data)
    .then(() => {
...
export const updateFetchTaskPremise = (data: UpdateTaskPremise) => async (dispatch: any) => {
  dispatch(taskPremiseFetchStart)
  fetchApi
    .PutFetch<UpdateTaskPremise>('http://localhost:8008/logtool/task_premise_disc', data)
    .then(() => {
      dispatch(getFetchTaskPremise())
    })
    .catch((err) => {
...
export const deleteFetchTaskPremise = (id: number) => async (dispatch: any) => {
  dispatch(taskPremiseFetchStart)
  fetchApi
    .DeleteFetch(`http://localhost:8008/logtool/task_premise_disc?params=${id}`)
    .then(() => {
      dispatch(getFetchTaskPremise())
    })
    .catch((err) => {
