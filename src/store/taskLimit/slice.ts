...
export const { taskLimitFetchStart, taskLimitFetchFailure, taskLimitFetchSuccess } =
  taskLimitSlice.actions

export const getFetchTaskLimits = () => async (dispatch: any) => {
  dispatch(taskLimitFetchStart)
  fetchApi
    .GetFetch<TaskLimit>('http://localhost:8008/logtool/task_limit')
    .then((res) => {
...
export const createFetchTaskLimit = (data: AddTaskLimit) => async (dispatch: any) => {
  dispatch(taskLimitFetchStart)
  fetchApi
    .PostFetch<AddTaskLimit>('http://localhost:8008/logtool/task_limit', data)
    .then((res) => {
...
export const updateFetchTaskLimit = (data: AddTaskLimit) => async (dispatch: any) => {
  dispatch(taskLimitFetchStart)
  fetchApi
    .PutFetch<AddTaskLimit>('http://localhost:8008/logtool/task_limit', data)
    .then((res) => {
      dispatch(getFetchTaskLimits())
    })
    .catch((err) => {
...
export const deleteFetchTaskLimit = (id: number) => async (dispatch: any) => {
  dispatch(taskLimitFetchStart)
  fetchApi
    .DeleteFetch(`http://localhost:8008/logtool/task_limit?params=${id}`)
    .then((res) => {
      dispatch(getFetchTaskLimits())
    })
    .catch((err) => {
