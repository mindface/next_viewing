...
import { getFetchTaskObjective, createFetchObjective, updateFetchObjective, deleteFetchObjective } from "../../store/taskObjective/slice";
      ...
      <CommonModal type={viewType} setMethods={resetData} ref={modalRef} >
        ...
        <div className="add-task">
          ...
          <div className="task-from">
            ...
            <div className="task-from__field">
              <label htmlFor="lankId" className="label">優先度</label>
              <input type="number" className='input' name="lankId" id="clear_item" defaultValue={lankId} onChange={(e) => {setLankId(Number(e.target.value))}} />
            </div>
            <div className="task-from__field">
      ...
      <div className="box-task__body">
        <ul className="list _flex_ __m-r-8">
          {ob.map((item:TaskObjective) => {
            return (
              <li className="item __radius" key={item.id} onClick={(e) => {viewData(item)}} >
