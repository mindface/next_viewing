...
import {
  getFetchTaskLimits,
  createFetchTaskLimit,
  updateFetchTaskLimit,
  deleteFetchTaskLimit,
} from '../../store/taskLimit/slice'
  ...
  return (
    <div className='box-task bord bord--plan'>
        ...
        <div className='add-task'>
          ...
          <div className='task-from'>
            <div className='task-from__field'>
              <p className='label' onClick={(e) => setViewSelect(true)}>
                {title}
              </p>
              {viewSelect && (
      ...
      <div className='box-task__body'>
        <ul className='list'>
          {plans.length === 0 ? (
            <li className='item'>nodata</li>
          ) : (
            plans.map((item: TaskLimit) => {
              return (
                <li
                  className='item _flex_s_b_'
                  key={`plans${item.id}`}
                  onClick={(e) => {
                    viewData(item)
                  }}
