...
function SectionMakeStructure() {
  ...
  const [makeStruct, setMakeStruct] = useState([cardArray[0], cardArray[1]])
  ...
  const AddItem = () => {
    if (selectItem !== '') {
      ...
      items.push({
        id: Number(makeStruct.length + 1),
        selectId: `select0${makeStruct.length + 1}`,
        content: selectItem,
      })
      setMakeStruct(items)
    } else {
      ...
      <div className='content __limit-w-l'>
                ...
                {taskSelect && <div className='task-select'>
                    {tasks.map((item: BaseTask) => (
                      <div
                        className='task'
                        key={Number(item.id)}
                        onClick={() => selectTask(Number(item.id), item.title)}
                      >
                        {item.title}
                      </div>
                    ))}
            ...
            {makeStruct.map((item: any) => {
              return (
                <div className='structure' key={item.selectId}>
                  <span
                    className='aid __radius __boxshadow'
                    onClick={() => {
                      removeItem(item.id)
                    }}
                  >
        ...
        <div className='content__selects'>
          ...
          <div className='selects__items _flex_'>
            {cardArray.map((item) => {
              return (
                <div key={`cardArray${item.selectId}`}>
                  <input
                    type='radio'
                    name='selectItem'
                    id={item.selectId}
                    className='input'
                    onChange={() => setSelectItem(item.content)}
                  />
          ...
          <button className='btn __radius __boxshadow' onClick={() => AddItem()}>
            追加
          </button>
        </div>
      </div>
