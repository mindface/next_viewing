...
function CommonModal({children,type,setMethods,closeBtn = true}:Props,ref:any) {
  ...
  return (
    <div className={`modal-outer${styleClass ? ' edit':''}`}>
      {!type && <button className="btn open" onClick={e => modalAction()}>open</button>}
      { type !== 'nobtn' ? <button className="btn open __radius" onClick={e => modalAction()}>
        <Image
          src={setIcons()}
          alt="add svg"
          width={15}
      ...
      { closeClass && <div className={`modal-transfer`}>
        <div className='modal-box __radius'>
          <div className="styler-switch">
            <input type="checkbox" className="input" name="styler" id="styler" onChange={() => {setStyleClass(!styleClass)}} />
            <label htmlFor="styler" className="label switch">edit スタイル</label>
           </div>
          {closeBtn && <button className="btn close" onClick={e => modalAction()}>close</button>}
